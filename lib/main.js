  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        // execute the linting
        return helpers.exec(atom.config.get('linter-circle.circleExecutablePath'), ['config', 'validate', '--skip-update-check'], {stream: 'stderr', allowEmptyStderr: true}).then(output => {
          let toReturn = [];

          output.split(/\r?\n/).forEach((line) => {
            // setup matchers for output parsing
            const lintMatches = /\d\. (\[#\/.*)/.exec(line);
